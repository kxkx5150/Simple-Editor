...
****************************************/
simpleEditor.ACE_EDITOR_USER_COMMANDS = function(ACE,SimpleEditor,secallback) {
    ...
    var commands = [
	    {
		    ...
		    name: "TabtoSpace",
		    exec: function(editor) {
		    	var tabsize = SimpleEditor.optobj.tabsize;
		    ...
		    exec: function(editor) {
				...
				var session = editor.getSession();
				var text = session.getValue();
				var replace = new RegExp(new Array(tabsize + 1).join(" "), "g");
    ...
    var lang = LANG;
    var config = CONFIG;
    if(secallback)secallback(commands);
};
setTimeout(function(){
	simpleEditor.callBackUserCommand();
},100)
